...
static void set_labels_free(cypher_astnode_t *self);


const struct cypher_astnode_vt cypher_set_labels_astnode_vt =
    { .name = "set labels",
      .detailstr = detailstr,
      .free = set_labels_free };
...
void set_labels_free(cypher_astnode_t *self)
{
...
{
    REQUIRE(cypher_astnode_instanceof(self, CYPHER_AST_SET_LABELS), -1);
    struct set_labels *node = container_of(self, struct set_labels, _astnode);
