...
const struct cypher_astnode_vt cypher_create_rel_prop_exists_constraint_astnode_vt =
    { .parents = parents,
      .nparents = 1,
      .name = "create rel prop existence constraint",
      .detailstr = detailstr,
        ...
        unsigned int nchildren, struct cypher_input_range range)
{
    ...
    if (cypher_astnode_init(&(node->_astnode),
            CYPHER_AST_CREATE_REL_PROP_EXISTS_CONSTRAINT,
...
{
    REQUIRE_TYPE(self, CYPHER_AST_CREATE_REL_PROP_EXISTS_CONSTRAINT, -1);
