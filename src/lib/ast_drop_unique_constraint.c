...
const struct cypher_astnode_vt cypher_drop_unique_constraint_astnode_vt =
    { .parents = parents,
      .nparents = 1,
      .name = "drop unique node prop constraint",
      .detailstr = detailstr,
        ...
        unsigned int nchildren, struct cypher_input_range range)
{
    ...
    if (cypher_astnode_init(&(node->_astnode),
            CYPHER_AST_DROP_UNIQUE_NODE_PROP_CONSTRAINT, children, nchildren, range))
    {
        free(node);
        return NULL;
    }
...
{
    REQUIRE_TYPE(self, CYPHER_AST_DROP_UNIQUE_NODE_PROP_CONSTRAINT, -1);
