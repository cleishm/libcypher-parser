...
const struct cypher_astnode_vt cypher_drop_node_prop_exists_constraint_astnode_vt =
    { .parents = parents,
      .nparents = 1,
      .name = "drop node prop existence constraint",
      .detailstr = detailstr,
        ...
        unsigned int nchildren, struct cypher_input_range range)
{
    ...
    if (cypher_astnode_init(&(node->_astnode),
            CYPHER_AST_DROP_NODE_PROP_EXISTS_CONSTRAINT,
...
{
    REQUIRE_TYPE(self, CYPHER_AST_DROP_NODE_PROP_EXISTS_CONSTRAINT, -1);
