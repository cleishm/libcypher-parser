...
[
  ...
  PACKAGE_PYTHON_VERSION="$PACKAGE_MAJOR_VERSION.$PACKAGE_MINOR_VERSION.$PACKAGE_PATCH_VERSION"

  if test "X$PACKAGE_DEVELOPMENT_VERSION" = "X"; then
    PACKAGE_STATUS_VERSION=0
  else
    PACKAGE_STATUS_VERSION=1
    PACKAGE_PYTHON_VERSION="$PACKAGE_PYTHON_VERSION.dev0"
  fi
  AC_SUBST([PACKAGE_STATUS_VERSION])
  AC_SUBST([PACKAGE_PYTHON_VERSION])
])
