...
START_TEST (parse_path_pattern)
{
    struct cypher_input_positionlast = cypher_input_position_zero;
...
"@14  22..31  > > > > > > identifier                    `my_cousin`\n";
    ...
    const cypher_astnode_t *path = cypher_ast_pattern_path_get_element(path, 1);
    ck_assert_int_eq(cypher_astnode_type(path), CYPHER_AST_PATH_PATTERN);
    
    ck_assert_int_eq(cypher_ast_path_pattern_get_direction(path),
            CYPHER_REL_BIDIRECTIONAL);
    
    const cypher_astnode_t *expr = cypher_path_pattern_get_expression(path);
    ...
    const cypher_astnode_t *alt = cypher_path_pattern_expression_get_element(expr, 0);
    ...
    ck_assert_int_eq(cypher_ast_path_pattern_alternative_get_element(alt, 1), NULL);
    
    const cypher_astnode_t *base = cypher_path_pattern_alternative_get_element(alt, 0);
    ...
    ck_assert_str_eq(cypher_ast_path_pattern_reference_get_identifier(ref), "cousin");
...
TCase* pattern_tcase(void)
{
    ...
    tcase_add_test(tc, parse_path_pattern);
    return tc;
}
