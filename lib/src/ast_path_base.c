...
const struct cypher_astnode_vt cypher_path_base_astnode_vt =
    { .name = "path base",
      .detailstr = detailstr,
      .release = cypher_astnode_release,
      .clone = clone };
      
cypher_astnode_t *cypher_ast_path_base(const cypher_astnode_t *identifier,
        enum cypher_rel_direction direction, cypher_astnode_t **children, unsigned int nchildren,
        struct cypher_input_range range)
{
    struct path_base *node = calloc(1, sizeof(struct path_base));
    if (node == NULL)
    {
        return NULL;
    }
    if (cypher_astnode_init(&(node->_astnode), CYPHER_AST_PATH_BASE,
