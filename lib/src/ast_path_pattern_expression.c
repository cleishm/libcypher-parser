...
cypher_astnode_t *cypher_ast_path_pattern_expression(
        const cypher_astnode_t *properties,
        cypher_astnode_t * const *elements, unsigned int nelements,
        cypher_astnode_t **children, unsigned int nchildren,
        struct cypher_input_range range)
{
    struct path_pattern_expression *node = calloc(1,
            sizeof(struct path_pattern_expression) +
    ...
    if (cypher_astnode_init(&(node->_astnode),
                CYPHER_AST_PATH_PATTERN_EXPRESSION, children, nchildren, range))
    {
        goto cleanup;
    }
...
unsigned int cypher_ast_path_pattern_expression_get_nelements(
...
cypher_astnode_t *clone(const cypher_astnode_t *self,
        cypher_astnode_t **children)
{
    struct path_pattern_expression *node = container_of(self,
            struct path_pattern_expression, _astnode);
    ...
    cypher_astnode_t *clone = cypher_ast_path_pattern_expression(properties,
            elements, node->nelements, children, self->nchildren, self->range);
    return clone;
}
