...
{
    REQUIRE_TYPE(self, CYPHER_AST_CREATE_NODE_PROP_CONSTRAINT, NULL);
    ...
    cypher_astnode_t *clone = cypher_ast_create_node_prop_constraint(identifier,
            label, expression, node->unique, children, self->nchildren,
            self->range);
    int errsv = errno;
    free(children);
    errno = errsv;
