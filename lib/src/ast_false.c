...
{
    struct false_literal *node = calloc(1, sizeof(struct false_literal));
    if (node == NULL)
    {
        return NULL;
    }
    if (cypher_astnode_init(&(node->_astnode), CYPHER_AST_FALSE,
            NULL, 0, range))
    {
        free(node);
        return NULL;
...
{
    REQUIRE_TYPE(self, CYPHER_AST_FALSE, NULL);
    return cypher_ast_false(self->range);
}
